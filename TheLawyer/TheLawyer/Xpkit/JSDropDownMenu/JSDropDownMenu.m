...
// 选中颜色加深
#define SelectColor [UIColor colorWithHex:0xF0F4FE]
#
...
- (CATextLayer *)createTextLayerWithNSString:(NSString *)string withColor:(UIColor *)color andPosition:(CGPoint)point {
    ...
    layer.bounds = CGRectMake(0, 0, sizeWidth +5, size.height+5);
    layer.string = string;
    layer.fontSize = 14.0;
...
#pragma mark--------------设置不可以点击的index---------------
- (void)setDisableIndex:(NSInteger)disableIndex{
    _disableIndex=disableIndex;
    _cannotSelectIndex=_disableIndex;
}
//、、点击item
...
- (void)menuTapped:(UITapGestureRecognizer *)paramSender {
     ...
     *  修改点击第一列的时候不作出任何反应
     */
    if (tapIndex==_cannotSelectIndex) {
        return;
    }
...
#pragma mark - 点击下面的部分 让table和collectionView 隐藏
 ...
 *动画显示下拉菜单
 */
#pragma mark - 动画显示collectionView的下拉菜单
...
- (void)confiMenuWithSelectRow:(NSInteger)row leftOrRight:(NSInteger)leftOrRight{
    ...
    indicator.position = CGPointMake(title.position.x + title.frame.size.width / 2 + 12, indicator.position.y);
}
